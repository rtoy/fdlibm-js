describe(
    "Tests of Math.exp for exceptional values",
    function () {
        it("Math.exp(Infinity) = Infinity",
           function () {
               var y = Math.exp(Infinity);
               expect(y).toBe(Infinity);
           });
        it("Math.exp(-Infinity) = 0",
           function() {
               var y = Math.exp(-Infinity);
               // Check for +0
...
describe(
    "Tests Math.exp basic functionality",
    function () {
        it("Math.exp(709.7822265625e0), no overflow",
           function () {
               var x = _ConstructDouble(0x40862e42, 0);
               var y = Math.exp(x);
               expect(y).toBe(1.7968190737295725e308);
           });
        it("Math.exp(-709.7822265625e0), no underflow",
           function () {
               var x = -_ConstructDouble(0x40862e42, 0);
               var y = Math.exp(x);
        ...
        it("Math.exp(0.5), case 0.5*log(2) < |x| < 1.5*log(2)",
           function () {
               y = Math.exp(0.5);
        ...
        it("Math.exp(2), case |x| > 1.5*log(2)",
           function () {
               y = Math.exp(2);
        ...
        it("Math.exp(2^-1022), case k < -1021",
           function () {
               var x = Math.pow(2,-1022);
               var y = Math.exp(x);
               expect(y).toBe(1);
           });
        it("Math.exp(2^-1021), case k >= -1021",
           function () {
        ...
        it("Math.exp(709.7827128933841d0), overflows",
           function () {
        ...
        it("Math.exp(-745.1332191019412), underflows",
           function () {
               var x = _ConstructDouble(0xc0874910, 0xD52D3051+1);
               var y = Math.exp(x);
               expect(y).toBe(0);
           });
        it("Math.exp(1000) = Infinity, overflow case",
           function () {
               var y = Math.exp(1000);
               expect(y).toBe(Infinity);
           });
        it("Math.exp(-1000) = 0, underflow case",
           function () {
               var y = Math.exp(-1000);
               expect(y).toBe(0);
           });
...
describe(
    "Test that Math.exp(x) is still monotonic near x = 1",
    ...
    function () {
        it("Math.exp(1) = Math.E",
           function () {
               var y = Math.exp(1);
               expect(y).toBe(Math.E);
           });
        it("Math.exp(1-negeps) <= Math.E",
           function () {
